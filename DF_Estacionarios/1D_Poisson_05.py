...
Ejecución:
        python 1D_Poisson_05.py entrada salida donde "entrada" es el nombre de 
        ...
        Por ejemplo: python 1D_Poisson_05.py INPUT_01 SALIDA
"""
    ...
    """
        ...
        Param
            ...
            Error: escalar
                Error obtenido.
    """
    PoissonLatex = 'Ecuacion de Poisson : $\partial^2 u(x)/\partial x^2 = \omega^2 u(x); \,\,\, u(a) = u(b) = 1$'
    ErrorLatex = 'Sol. Numerica : $E(h = %g) = %g$' % (h, Error)
    SolLatex = 'Sol. Analitica : $u(x) = \\frac{1 - \cos(\omega)}{\sin(\omega)} \sin(\omega x) + \cos(\omega x)$'

    plt.plot(x,u,'--o',label=ErrorLatex)
    plt.plot(xa,ua,'-r',label=SolLatex)
    plt.title(PoissonLatex)
    plt.xlabel("$x$")
    plt.ylabel("$u$")
    plt.ylim(-2.0,3.0)
    plt.legend(loc='upper right')
    plt.grid()
    plt.savefig("1D_Poisson_05.pdf")
    plt.show()
...
try:
    ...
    mensaje = """ Error: La ejecucion de este programa requiere de 2 argumentos. 
        Ejecucion correcta: python 1D_Poisson_05.py entrada salida
...
Error = np.sqrt(h) * np.linalg.norm(Evalua(x,w) - u)
...
ua = Evalua(xa,w)
GraficaSolucion(x,u,xa,ua,h,Error)
